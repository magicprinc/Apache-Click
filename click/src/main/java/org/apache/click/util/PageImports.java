...
public class PageImports {
  ...
  public void populateTemplateModel(Map<String, Object> model) {
    ...
    if (pop != null){
      logger.warn(page.getClass().getName() + " on " + page.getPath()
  ...
  public void processControl(Control control) {
    ...
    if (control instanceof Panel panel) {
      if (!panel.isActive()) {
        return;
      }
    }
    ...
    if (control instanceof Container container) {
    ...
    } else if (control instanceof RadioGroup radioGroup) {
  ...
  public final List<Element> getHeadElements() {
    List<Element> result = new ArrayList<>(headElements);
  ...
  public final List<Element> getJsElements() {
    List<Element> result = new ArrayList<>(jsImports);
