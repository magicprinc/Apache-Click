...
public class Form extends AbstractContainer implements Stateful {
  ...
  public boolean onProcess () {
    ...
    if (hasPostError()){
      ...
      Context.getThreadLocalContext().getRequest().removeAttribute(FileUploadService.UPLOAD_EXCEPTION);
  ...
  public void render (HtmlStringBuffer buffer) {
    final boolean process =
        Context.getThreadLocalContext().getRequest().getMethod().equalsIgnoreCase(getMethod());
  ...
  protected boolean hasPostError () {
    Exception e = (Exception)
        Context.getThreadLocalContext().getRequest().getAttribute(FileUploadService.UPLOAD_EXCEPTION);

    return e instanceof FileSizeLimitExceededException
  ...
  protected void validateFileUpload () {
    ...
    Exception exception = (Exception) Context.getThreadLocalContext().getRequest()
        .getAttribute(FileUploadService.UPLOAD_EXCEPTION);

    if (!( exception instanceof FileUploadException fue )){
      return;
    }
    ...
    if (fue instanceof SizeLimitExceededException se){
    ...
    } else if (fue instanceof FileSizeLimitExceededException fse){
