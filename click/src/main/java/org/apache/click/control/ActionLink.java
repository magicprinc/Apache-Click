...
public class ActionLink extends AbstractLink {
  private static final long serialVersionUID = -5025910359741041085L;
   ...
   */
  public ActionLink (@NonNull Object listener, @NonNull String method) {
   ...
   * or if the method is blank
   */
  public ActionLink (String name, @NonNull Object listener, @NonNull String method) {
    setName(name);
    if (StringUtils.isBlank(method)) {
      throw new IllegalArgumentException("Blank listener method");
    }
   ...
   * @param listener the listener target object
   * @param method the listener method to call
   * @throws IllegalArgumentException if the name, listener or method is null or if the method is blank
   */
  public ActionLink(String name, String label, @NonNull Object listener, @NonNull String method){
    setName(name);
    setLabel(label);
    if (StringUtils.isBlank(method)) {
      throw new IllegalArgumentException("Blank listener method");
  ...
  public ActionLink (@NonNull ActionListener actionListener){
    setActionListener(actionListener);
  }//new

  public ActionLink (@NonNull String name, @NonNull ActionListener actionListener){
    setName(name);
    setActionListener(actionListener);
  }//new


  public ActionLink (@NonNull String name, String label, @NonNull ActionListener actionListener){
    setName(name);
    setLabel(label);
    setActionListener(actionListener);
  }//new
        ...
        if (!paramName.equals(ACTION_LINK) && !paramName.equals(VALUE)) {
          ...
          if (paramValue instanceof String[] paramValues) {
            for (String s : paramValues){
              ...
              buffer.append("=");
              buffer.append(ClickUtils.encodeUrl(s,
                  context));
            }
          } else {
            if (paramValue != null) {
