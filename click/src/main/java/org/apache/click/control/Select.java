...
public class Select extends Field {
  ...
  @Getter protected DataProvider dataProvider;
  ...
  public void addAll (@NonNull Map options) {
    for (Object o : options.entrySet()){
      ...
      Option option = new Option(entry.getKey().toString(), entry.getValue().toString());
  ...
  public List<String> getSelectedValues() {
    return selectedValues != null ? selectedValues : Collections.emptyList();
  }
  ...
  public List getOptionList() {
    if (optionList == null) {
      ...
      if (dp != null) {
        ...
        if (iterableData instanceof List listData) {
  ...
  public void bindRequestValue() {
    ...
    List<String> localSelectedValues = new ArrayList<>(5);

    // Process single item select case.
    if (!isMultiple()) {
      // Load the selected item.
      value = Context.getThreadLocalContext().getRequestParameter(getName());
      ...
      String[] parameterValues =
          Context.getThreadLocalContext().getRequest().getParameterValues(getName());

      if (parameterValues != null) {
        Collections.addAll(localSelectedValues, parameterValues);
      }
    }
  ...
  public void setState(Object state) {
    ...
    List<String> localSelectedState;

    if (state instanceof String selectState) {
      localSelectedState = new ArrayList<>(1);
      setValue(selectState);
      localSelectedState.add(selectState);
    } else {
      String[] selectState = (String[]) state;
      localSelectedState = new ArrayList<>(selectState.length);
      Collections.addAll(localSelectedState, selectState);
    }
      ...
      if (isMultiple()) {
        ...
        if (getValue().length() == 0) {
          ...
          if (defaultValue == null) {
            ...
            if (firstEntry instanceof Option option) {
  ...
  protected void setInitialValue() {
    if ((getValue().length() == 0) && !getOptionList().isEmpty()) {
      ...
      if (object instanceof String) {
      ...
      } else if (object instanceof Option option) {
        ...
        if (!optionGroup.getChildren().isEmpty()) {
          ...
          } else if (child instanceof OptionGroup childOptionGroup) {

            if (!childOptionGroup.getChildren().isEmpty()) {
              Object cogChild = childOptionGroup.getChildren().get(0);

              if (cogChild instanceof Option option) {
                setValue(option.getValue());
              }
            }
          }
