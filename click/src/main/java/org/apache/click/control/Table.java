...
public class Table extends AbstractControl implements Stateful {
  ...
  @Getter @Setter protected String height;

  /** The table data provider (table row list DataProvider). */
  ...
  @Override public Object getState() {
    ...
    if (currentPageNumber != 0) {
      hasState = true;
      tableState[0] = currentPageNumber;
    }
    ...
    if (!currentSortedAscending) {
      hasState = true;
      tableState[2] = currentSortedAscending;
    }
  ...
  @Override public void setState(Object state) {
    ...
    if (tableState[0] != null) {
      int storedPageNumber = (Integer) tableState[0];
    ...
    if (tableState[2] != null) {
      boolean storedAscending = (Boolean) tableState[2];
  ...
  protected List<Object> createRowList() {
    ...
    if (dp != null) {
      ...
      boolean isPaginating = dp instanceof PagingDataProvider<?>;
  ...
  protected void renderHeaderRow(HtmlStringBuffer buffer) {
    ...
    for (int j = 0; j < tableColumns.size(); j++) {
      ...
      column.renderTableHeader(buffer, Context.getThreadLocalContext());
  ...
  protected void renderBodyRowColumns(HtmlStringBuffer buffer, int rowIndex) {
    ...
    for (int j = 0; j < tableColumns.size(); j++) {
      Column column = tableColumns.get(j);
      column.renderTableData(row, buffer, Context.getThreadLocalContext(), rowIndex);
      if (j < tableColumns.size() - 1) {
        buffer.append("\n");
      }
    }
