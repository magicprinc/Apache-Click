...
public class Step2 extends Step {
    ...
    private final Select stateSelect;
    ...
    public void onInit() {
        ...
        String postCodeValue = Context.getThreadLocalContext().getRequestParameter(postCodeField.getName());
        if (postCodeValue != null) {
            postCodeField.setValue(postCodeValue);
        }
        // Check if state is passed to this Page from SelectPostCode page
        String stateValue = Context.getThreadLocalContext().getRequestParameter(stateSelect.getName());
        if (stateValue != null) {
