...
public class StyledOptionPage extends BorderPage {
  ...
  private final Grid     grid2             = new Grid("grid2");
  private final Reset    sbReset           = new Reset("Reset values");
  private final Checkbox chJavascript      = new Checkbox("javascriptToggle");

  private final Select   backgroundcolor   = new Select("a");
  private final Select   backgroundcolor_w = new Select("w_b");
  private final Select   color             = new Select("c");
  private final Select   color_w           = new Select("w_d");
  private final Select   utf8              = new Select("e");
  private final Select   utf8_w            = new Select("w_f");
  private final Select   firstLetter       = new Select("g");
  private final Select   firstLetter_w     = new Select("w_h");
  private final Select   images            = new Select("i");
  private final Select   images_w          = new Select("w_j");
  private final Select   font              = new Select("k");
  private final Select   font_w            = new Select("w_l");
  private final Select   fontSize          = new Select("m");
  private final Select   fontSize_w        = new Select("w_n");
  ...
  private List<StyledOption> optionCreator(String[] names) {
    val opts = new ArrayList<StyledOption>(names.length);
    for (String name : names){
      opts.add(new StyledOption("" + ++optionValue, name));
    }
    return opts;
  }

  private List<StyledOption> optionCreator(String[] labels, String[] names) {
