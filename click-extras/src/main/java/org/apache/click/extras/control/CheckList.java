...
public class CheckList extends Field {
  ...
  @Getter protected DataProvider<Option> dataProvider;
   ...
   */
  @Override public String getTag (){ return "div";}
  ...
  public void addAll (@NonNull Collection<?> objects, @NonNull String optionValueProperty, String optionLabelProperty) {
    ...
    for (Object object : objects){
      try {
        Object valueResult = propertyService.getValue(object, optionValueProperty, methodCache);

        // Default labelResult to valueResult
        Object labelResult = valueResult;

        // If optionLabelProperty is specified, lookup the labelResult from the object
        if (optionLabelProperty != null) {
          labelResult = propertyService.getValue(object, optionLabelProperty, methodCache);
        }
   ...
   */
  @Override public void bindRequestValue() {
    Context context = Context.getThreadLocalContext();
    ...
    List<String> localSelectedValues = new ArrayList<>();
    ...
    if (isSortable()) {
      ...
      if (orderParameterValues != null) {
        this.sortorder = new ArrayList<>(orderParameterValues.length);
  ...
  protected void sortOptions(String[] order) {
    ...
    final List<Option> orderList = new ArrayList<>(options.size());
  ...
  public void render(HtmlStringBuffer buffer) {
    ...
    if (!optionsList.isEmpty()) {
      ...
      for (Option option : optionsList) {
        ...
        for (String s : values){
          if (String.valueOf(s).equals(option.getValue())){
            checked = true;
            break;
          }
        }
