...
public class SubmitLink extends ActionLink {
  ...
  public String getHref(Object value) {
    ...
    if (hasParameters()) {
      for (String localName : getParameters().keySet()) {
        if (!localName.equals(ACTION_LINK) && !localName.equals(VALUE)) {
          ...
          if (paramValue instanceof String[] paramValues) {
            for (String s : paramValues){
              renderParameter(localName, s, prefix, buffer, context);
            }
          } else {
            renderParameter(localName, paramValue, prefix, buffer, context);
          }
        }
      }
    }
  ...
  public List<Element> getHeadElements() {
    ...
    if (headElements == null) {
      ...
      String versionIndicator = ClickUtils.getResourceVersionIndicator(Context.getThreadLocalContext());

      JsImport jsImport = new JsImport("/click/extras-control.js", versionIndicator);
