...
public class MockTests extends TestCase {
  ...
  public void testMockServletConfig() {
    ...
    assertEquals(value, servletConfig.getInitParameter(key));
    assertEquals(servletName, servletConfig.getServletName());
    assertEquals(servletContext, servletConfig.getServletContext());
  }
  ...
  public void testMockServletContext() {
    try {
      ...
      assertEquals(value, servletContext.getInitParameter(key));
      assertEquals(contextPath, servletContext.getContextPath());
  ...
  public void testTemporaryDirectorySetup() {
    try {
      ...
      assertTrue(convertedTemp.exists());
  ...
  public void testDeleteDirectoryOnShutdown() {
    ...
    try {
      ...
      tmpDir.mkdir();
      assertTrue(tmpDir.isDirectory() && tmpDir.canWrite());

      final File subDir = new File(tmpDir, "subdir");
      subDir.mkdir();
      assertTrue(subDir.isDirectory() && subDir.canWrite());
      ...
      ex.printStackTrace();
      fail("Temporary directory must be deleted");
    }
  }
}