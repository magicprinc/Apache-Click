...
public class MockContainer {
	 ...
	 */
	@Setter @Getter private MockRequest request;
	 ...
	 */
	@Setter @Getter private MockResponse response;
	 ...
	 */
	@Getter @Setter private ClickServlet clickServlet;
	 ...
	 */
	@Getter @Setter private MockServletConfig servletConfig;
	...
	/** Holds the MockSession instance.
		 ...
		 */
	@Setter @Getter private MockSession session;
	...
	public static MockRequest findMockRequest (ServletRequest request) {
		...
		if (request instanceof MockRequest mr){
			return mr;
		} else {
	...
	void configure () {
		try {
			...
			if (getServletConfig() == null) {
				...
				setServletConfig(new MockServletConfig(servletName,	getServletContext()));
			}
			...
			getServletContext().setAttribute(ClickServlet.MOCK_MODE_ENABLED, Boolean.TRUE);
	...
	public MockContainer pages (String pagePackage) {
		getServletContext().addInitParameter("pages", pagePackage);
		return this;
	}

	/** A RuntimeException that only prints the original cause, instead of printing nested stackTraces. */
